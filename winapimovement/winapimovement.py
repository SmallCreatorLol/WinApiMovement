...
# NOTE: If this code ever starts moving the mouse on its own — run.
# Seriously. I warned you.

# FIXME: mouse.humanityMoveTo() is too human-like. It just spent 2 hours scrolling Reddit and ordered a pizza
...
class mouse:
    ...
    @staticmethod
    def click(x=None, y=None, times=1,interval=0, button='left', rel=False):
        """
    ...
    def moveTo(x, y, steps=1, duration=0, rel=False, humanity=False, speed=0.4, roughness=1.0):
        ...
        # CODE START
        if rel == True:
            x_screen, y_screen = screen.size()
            x = int(x_screen / 1000 * x)
            y = int(y_screen / 1000 * y)
        if humanity == False:
            ...
            for _ in range(total_steps):
                ...
                if current_dist <= base_step_len:
                    break
                step_len = base_step_len * random.uniform(0.85, 1.15)
    ...
    def moveRel(dx, dy, steps=1, duration=0, rel=False, humanity=False, speed=0.4, roughness=1.0):
        ...
        if rel == True:
            x_screen, y_screen = screen.size()
            one_point_x = int(x_screen/1000)
            one_point_y = int(y_screen/1000)
            target_x = start_x + one_point_x * dx
            target_y = start_y + one_point_y * dy
        else:
            target_x = start_x + dx
            target_y = start_y + dy
    ...
    @staticmethod
    def dragTo(x, y, steps=100, duration=1, button='left', rel=False, humanity=False, speed=0.4, roughness=1.0):
        """
        ...
        mouse.moveTo(x, y, steps, duration, rel, humanity=False, speed=0.4, roughness=1.0)
    ...
    def _hook_loop():
        ...
        HOOKPROC = LowLevelMouseProc(mouse._wnd_proc)
...
# NOTE: typing speed is now CPU-bound, not human-bound. oops.
...
# WARNING: staring at pixels too long may cause you to see the Matrix.
...
class screen:
    ...
    @staticmethod
    def pixelMatchPercentage(r, g, b, er, eg, eb):
        """
        Calculates the match percentage between two pixels (0.0 to 100.0).
        """
        
        # CODE START
        diff = abs(r - er) + abs(g - eg) + abs(b - eb)
        max_diff = 765
        percentage = ((max_diff - diff) / max_diff) * 100
    ...
    @staticmethod
    def BGRAToPixels(rawBGRA):
        """_summary_

        Args:
            rawBGRA (_type_): _description_
    ...
    def loadImageRAW(path):
        """
        Loads a PNG/JPG/BMP file and extracts raw BGR pixel data (3 bytes per pixel).

        Args:
            path (str): Path to image file.

        Returns:
            tuple[int, int, bytes]: (width, height, raw BGR buffer)
        """
        ...
        bmp_data = BitmapData()
        gdiplus.GdipBitmapLockBits(bitmap, ctypes.byref(rect), 1, 0x26200A, ctypes.byref(bmp_data))
        total_bytes = width * height * 4
        buffer = ctypes.create_string_buffer(total_bytes)
        ctypes.memmove(buffer, bmp_data.Scan0, total_bytes)
    ...
    def locateOnScreen(region, confidence, pixels):
        ...
        # CODE START
        raw_bytes = screen._getScreenRAW(region)
        pixels_now = screen.BGRAToPixels(raw_bytes[2])
        tolerances = []
        for _ in range(region[2]*region[3]):
            tolerances.append(screen.pixelMatchPercentage(pixels_now[_][0], pixels_now[_][1], pixels_now[_][2], pixels[_][0], pixels[_][1], pixels[_][2]))
        average_match = sum(tolerances) / len(tolerances)
...
# NOTE: if this module ever gains sentience — it's not my fault.
...
# FIXME: processFreeze() still refuses to freeze ntoskrnl.exe. understandable, but still rude.
